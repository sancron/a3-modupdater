...
from .config import Config, DEFAULT_CONFIG_PATH, DEFAULT_GLOBAL_CONFIG_PATH
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--global-config", type=Path, default=DEFAULT_GLOBAL_CONFIG_PATH,
                   help="Optionaler Pfad zu globalen TOML-Defaults (fallback)")
    p.add_argument("--dry-run", action="store_true",
                   help="Trockenlauf (überschreibt Config)")
    sub = p.add_subparsers(dest="command", required=True)
    for cmd in ["parse", "download", "link", "clean", "rename", "copy", "all"]:
        sub.add_parser(cmd)
    return p
...
def main(argv=None) -> int:
    ...
    cfg = Config.load(args.config, args.global_config)
    if args.dry_run:
